        ...
        (t (destructuring-bind (op arg) (car instructions)
             (ecase op
               ...
               ((:keep-tail) (diff-score (cdr instructions)
                                         (1+ acc)))
               ((:replace-tail) (diff-score (cdr instructions)
                                            (+ acc 1 (tree-length arg))))
               ((:add-tail) (diff-score (cdr instructions)
                                        (+ acc (tree-length arg)))))))))
  ...
  (cond ((and (null seq1) (null seq2))
        ...
        ((not (listp seq1))
         (cond ((or (null seq2) (consp seq2))
                (longest-subseq nil seq2 (cons (list :drop-tail 1)
                                               acc)))
               ((equalp seq1 seq2)
                (longest-subseq nil nil (cons (list :keep-tail 1)
                                              acc)))
...
(defun list-diff (list1 list2)
  (labels ((combine (seq)
             ...
             (ecase (-> seq first first)
               ...
               ((:into :keep-tail :add-tail :drop-tail :replace-tail) (first seq))))
           (rle-compress (diff)
             (-> diff
                 (group-by $ :predicate (lambda (e1 e2)
                                          (cond ((and (consp e1)
                                                      (consp e2)
                                                      (-> e1 first (member $ '(:into
                                                                               :keep-tail
                                                                               :add-tail
                                                                               :drop-tail
                                                                               :replace-tail)) not))
...
(defun list-patch (list diff)
  (cond ((null diff) nil)
        (t (destructuring-bind (op arg) (car diff)
             (ecase op
               ((:keep) (append (subseq list 0 arg)
                                (list-patch (subseq list arg)
                                            (cdr diff))))
               ((:add) (append arg (list-patch list
                                               (cdr diff))))
